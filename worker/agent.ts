...
export class ChatAgent extends Agent<Env, ChatState> {
   ...
   */
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean; systemPrompt?: string }): Promise<Response> {
    const { message, model, stream, systemPrompt } = body;
    // Validate input
    if (!message?.trim()) {
      return Response.json({
        success: false,
    ...
    // Don't persist demo messages to history
    const shouldPersist = !systemPrompt;
    if (shouldPersist) {
      this.setState({
        ...this.state,
